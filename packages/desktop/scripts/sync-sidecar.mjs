#!/usr/bin/env node
/**
 * 把 `dist/chariot-server.exe`(由 scripts/build.py 产出)同步到 Tauri 期待的
 * `tauri/binaries/chariot-server-<host-triplet>.exe` 位置。
 *
...
const SRC_EXE = resolve(REPO_ROOT, "dist", "chariot-server.exe");
...
function main() {
  ...
  const destExe = resolve(DEST_DIR, `chariot-server-${triplet}.exe`);
