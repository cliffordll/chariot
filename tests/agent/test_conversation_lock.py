"""ConversationLockManager + ConversationRepo.with_advisory_lock 单测(0.6.0 双层锁)。

覆盖:
- 进程内并发:`asyncio.gather` 起 N 个并发 `acquire(conv_id)`,critical section
...
- 双层 acquire:`acquire(conv_id, db_session=s)` 同时持进程内锁 + DB 锁
...
class TestInProcessSerialization:
    """同进程内同 conv_id 的并发 acquire 实际串行。"""
    ...
    async def test_concurrent_acquires_serialize(self) -> None:
        ...
        async def worker(idx: int) -> None:
            ...
            async with ConversationLockManager.acquire("conv_x"):
                active += 1
    ...
    async def test_different_conv_ids_parallel(self) -> None:
        """不同 conv_id 各自有锁,可并行。"""
        active = 0
        max_active = 0

        async def worker(conv_id: str) -> None:
            nonlocal active, max_active
...
# ---------------------------------------------------------------------------
# ConversationRepo.with_advisory_lock 基础行为
# ---------------------------------------------------------------------------
...
class TestAdvisoryLockCrossConnection:
    ...
    async def test_second_acquire_times_out_when_first_holds(
        ...
        应在重试耗尽后抛 ConversationLockTimeout(layer="db")。
        """
        ...
        try:
            repo_b = ConversationRepo(session_b)
            with pytest.raises(ConversationLockTimeout) as exc_info:
...
class TestDoubleLayerAcquire:
    """`ConversationLockManager.acquire(conv_id, db_session=s)` 同时持两层锁。"""
    ...
    async def test_acquire_with_db_session_writes_and_commits(
        ...
        # 退出后,数据可见
        conv = await ConversationRepo(session_a).get("conv_dl")
        assert conv is not None
    ...
    async def test_acquire_without_db_session_local_only(self, session_a: AsyncSession) -> None:
        ...
        """
        async with ConversationLockManager.acquire("conv_local"):
            repo = ConversationRepo(session_a)
            await repo.create("conv_local", title="local-only")
        conv = await ConversationRepo(session_a).get("conv_local")
        assert conv is not None


# ---------------------------------------------------------------------------
# ConversationLockTimeout 字段
# ---------------------------------------------------------------------------


class TestExceptionFields:
    """ConversationLockTimeout 携带 layer 字段,surface 层据此映射 error_type。"""
