...
覆盖:
- 进程内并发:`asyncio.gather` 起 N 个并发 `acquire(conversation_id)`,critical section
...
- 双层 acquire:`acquire(conversation_id, db_session=s)` 同时持进程内锁 + DB 锁
...
class TestInProcessSerialization:
    """同进程内同 conversation_id 的并发 acquire 实际串行。"""
    ...
    async def test_different_conversation_ids_parallel(self) -> None:
        """不同 conversation_id 各自有锁,可并行。"""
        active = 0
        max_active = 0
