...
class TestHttpGetToolCreate:
    def test_accepts_canonical_allowed_domains_list(self) -> None:
        tool = HttpGetTool.create(_entry({"allowed_domains": ["example.com"], "max_bytes": 1024}))
    ...
    def test_accepts_json_array_string(self) -> None:
        tool = HttpGetTool.create(_entry({"allowed_domains": '["example.com","openai.com"]', "max_bytes": 1024}))
