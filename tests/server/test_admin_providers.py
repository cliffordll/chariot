"""/admin/providers 端点测试 —— GET 列表 / 探针 / entries CRUD。

0.3.1 路由模型重构后(active 概念删除):
- POST /admin/providers 切 active 端点已删除
- GET /admin/providers 不返 active 字段;entries 加 params
- entries CRUD 加 params 字段;delete 任意 entry 都允许(无 cannot_delete_active)

本文件覆盖:
- GET /admin/providers 列表(空 / 有 entries / params 字段)
- POST /admin/providers/{name}/probe 探针
- POST /admin/providers/entries 创建(params / 409 / 400)
- PUT /admin/providers/entries/{name} 更新(改 options / params 都立刻 rebuild)
- DELETE /admin/providers/entries/{name} 删除
- POST /admin/providers/entries/{name}/copy 复制(含 params)
"""
...
async def test_get_models_with_entries(admin_client: AsyncClient, session: AsyncSession) -> None:
    ...
    r = await admin_client.get("/admin/providers")
...
# ---------- /admin/providers/{name}/probe POST ----------
...
async def test_probe_mock_entry_returns_ok(
    ...
    r = await admin_client.post("/admin/providers/m1/probe")
    assert r.status_code == 200
    body = r.json()
...
async def test_probe_unknown_name_returns_404(
    ...
    r = await admin_client.post("/admin/providers/ghost/probe")
    assert r.status_code == 404
    assert "ghost" in r.text


# ---------- /admin/providers/entries POST(create)----------
...
async def test_create_entry_persists_and_refreshes_config(
    ...
    # DB 持久化
    entry = await ProviderRepo(session).get_entry("new-claude")
...
# ---------- /admin/providers/entries/{name} PUT(update)----------
...
async def test_update_entry_options(admin_client: AsyncClient, session: AsyncSession) -> None:
    ...
    r = await admin_client.put(
        "/admin/providers/entries/x",
        json={"options": {"a": 999}},
    )
    assert r.status_code == 200
    assert r.json()["options"] == {"a": 999}

    entry = await ProviderRepo(session).get_entry("x")
...
# ---------- /admin/providers/entries/{name} DELETE ----------
...
async def test_delete_entry_removes_row(admin_client: AsyncClient, session: AsyncSession) -> None:
    ...
    r = await admin_client.delete("/admin/providers/entries/b")
    assert r.status_code == 204
    assert await ProviderRepo(session).get_entry("b") is None
    # Agent.models 字典同步
...
async def test_delete_only_entry_succeeds(admin_client: AsyncClient, session: AsyncSession) -> None:
    ...
    r = await admin_client.delete("/admin/providers/entries/a")
    assert r.status_code == 204
...
async def test_delete_unknown_entry_returns_404(admin_client: AsyncClient) -> None:
    ...
    r = await admin_client.delete("/admin/providers/entries/ghost")
    assert r.status_code == 404


# ---------- /admin/providers/entries/{name}/copy POST ----------
...
async def test_duplicate_entry_default_name(
    ...
    r = await admin_client.post("/admin/providers/entries/src/copy", json={})
    assert r.status_code == 201
    body = r.json()
    ...
    # DB 也有
    entry = await ProviderRepo(session).get_entry("src_copy")
...
async def test_duplicate_entry_explicit_as_name(
    ...
    r = await admin_client.post(
        "/admin/providers/entries/src/copy",
        json={"as": "my-copy"},
    )
    assert r.status_code == 201
...
async def test_duplicate_unknown_src_returns_404(admin_client: AsyncClient) -> None:
    ...
    r = await admin_client.post("/admin/providers/entries/ghost/copy", json={})
    assert r.status_code == 404
