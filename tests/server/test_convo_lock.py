...
async def test_same_conv_second_blocks_until_first_release() -> None:
    ...
    async def first() -> None:
        async with ConvoLockManager.acquire(CONV_A, timeout_s=5):
...
async def test_lock_released_after_exit_allows_next() -> None:
    ...
    async with ConvoLockManager.acquire(CONV_A, timeout_s=1):
        pass
    # 退出后再 acquire 不应阻塞
    async with ConvoLockManager.acquire(CONV_A, timeout_s=1):
        pass
...
async def test_different_convs_do_not_block_each_other() -> None:
    ...
    async def hold_b() -> None:
        async with ConvoLockManager.acquire(CONV_B, timeout_s=5):
            order.append("b_in")
            await asyncio.sleep(0.05)
            order.append("b_out")
...
async def test_timeout_raises_service_error_503() -> None:
    """同 conv 第一把锁迟迟不放,第二个超时 → ServiceError(503, convo_busy)。"""
    ...
    async def waiter() -> None:
        ...
        with pytest.raises(ServiceError) as exc:
            async with ConvoLockManager.acquire(CONV_A, timeout_s=0.05):
                pass  # pragma: no cover · 不应进入
        assert exc.value.status == 503
        assert exc.value.code == "convo_busy"
        # 让 holder 收尾
        holder_done.set()
...
def test_env_default_timeout_when_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    """env 没设 → 30.0 默认。"""
    monkeypatch.delenv("CHARIOT_CONVO_LOCK_TIMEOUT_S", raising=False)
    assert ConvoLockManager._read_timeout_env() == 30.0


def test_env_invalid_falls_back_to_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """非数字 / 非正 → fallback。"""
    monkeypatch.setenv("CHARIOT_CONVO_LOCK_TIMEOUT_S", "not-a-number")
    assert ConvoLockManager._read_timeout_env() == 30.0
    monkeypatch.setenv("CHARIOT_CONVO_LOCK_TIMEOUT_S", "0")
    assert ConvoLockManager._read_timeout_env() == 30.0
    monkeypatch.setenv("CHARIOT_CONVO_LOCK_TIMEOUT_S", "-5")
    assert ConvoLockManager._read_timeout_env() == 30.0


def test_env_valid_value_taken(monkeypatch: pytest.MonkeyPatch) -> None:
    """有效正数生效。"""
    monkeypatch.setenv("CHARIOT_CONVO_LOCK_TIMEOUT_S", "10.5")
    assert ConvoLockManager._read_timeout_env() == 10.5
