"""ConvoRepo 测试 —— CRUD + cascade delete + seq 单调 +
    ...
    rows = (
        (
            await session.execute(
                select(MessageRow).where(MessageRow.convo_id == ULID_A),
            )
        )
        .scalars()
        .all()
...
async def test_append_message_seq_monotonic(session: AsyncSession) -> None:
    ...
    m2 = await repo.append_message(ULID_A, "assistant", "2", provider_name="mock")
...
async def test_append_message_model_name_only_assistant(
    ...
    repo = ConvoRepo(session)
    await repo.create(ULID_A)
    with pytest.raises(ConfigError, match="provider_name"):
        await repo.append_message(ULID_A, "user", "hi", provider_name="mock")
...
async def test_append_assistant_updates_last_model(session: AsyncSession) -> None:
    ...
    await repo.append_message(ULID_A, "assistant", "答", provider_name="claude")
    got = await repo.get(ULID_A)
    assert got is not None
...
async def test_load_messages_seq_order(session: AsyncSession) -> None:
    ...
    await repo.append_message(ULID_A, "assistant", "2", provider_name="mock")
