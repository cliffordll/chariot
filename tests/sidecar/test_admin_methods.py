...
闁?`test_chat_method.py` 閻熸粏鍩囬崹娲_焵椤戞寧顦风紒鏃€鎸抽幊?闂備緡鍠曠划娆撳焵椤掍椒浜㈢紒?JsonRpcServer dispatch 闁荤姍鍕_棆妞ゅ浚鍓熷畷姘跺箥椤曞懍绱?,
婵炶揪绲藉Λ娑㈠极閵堝_鍎?AIAgent.bootstrap + tmp DB(闂佸吋婢橀惌鍌氼焽?mock agent)闂佺偨鍎查弻锟犲焵?闂佹悶鍎虫慨宕囨嫻?admin methods
闂傚倸娲犻崑鎾绘偡閺囨俺鍏屾繝鈧?session_maker / repos 闂佸綊娼х粔鐑藉礂濡_吋宕?CRUD闂?

闁荤喐娲栧Λ娑樏?14 婵?method 闂佸憡鑹剧€氼垶宕靛_鍫熷剭?happy path + 闂佺_绻戞繛濠囧极椤撶喐缍囬柛锔诲幗濞?
...
闂備焦瀵ч悷銊╊敋閵堝_鍎樺ù锝堫潐琛奸柣?NOT_FOUND / DUPLICATE / INVALID_PARAMS 闂?1-2 婵?case闂?
"""
...
async def _call(
    ...
    """闁荤姷鍎ら崹宕囩博閺夋垟鏋?RPC,闁哄_鏅滈弻銊ッ洪弽顐ょ當妞ゆ垼娉曢_閬嶆偨椤栧棗鏌婇幒妤€鍑?response 闂?error)闂?""
...
async def _isolate() -> AsyncIterator[None]:
    """濠?test 闂佸憡鎸哥粔鎾_箖濡も偓閵?AgentRegistry + DB engine,test 闂傚倸鍊搁悺銊ノ涢妶鍡欌枖闁圭粯甯掗柊閬嶆煏?""
...
async def agent(tmp_path: Path) -> AIAgent:
    """濠?test 婵炴垶鎸撮崑鎾斥槈閹垮啩绨婚柛娆忕箻瀵?DB + 闂佺_绻堥崝宥夊蓟?AIAgent;seed 闁荤姷鍎ら崹鐢告偩椤掑嫬瑙﹂幖绮瑰墲閸?mock entry + 4 disabled tool闂?""
...
def server(agent: AIAgent, tmp_path: Path) -> JsonRpcServer:
    """閻庡湱顭堝_鍫曞极閻愬搫绀冮悘鐐舵_瀵_灝鈹?method 闂?JsonRpcServer 闁诲骸婀遍崑妯兼_閵夆晛违?

    `db_path` 缂?ChatMethod 闂?0.6.6+ per-call override 闁荤姳璀﹂崹鎵__?;admin methods
    闁荤姷鍎ら崹鐟扳枔閹寸偟鈻旂€广儱鐗愬▔鏌ユ⒑椤撱劎绋绘繛纰卞灣閹瑰嫰顢涘_杈╃嵁,婵?register_methods 缂備焦绋掗崕鎶藉箖閺囩姵鍟哄ù锝呮贡濠€?闁哄_鏅滈悷鈺呭闯妞嬪孩宕?agent fixture 闂佸憡鑹鹃張顒傝姳椤曗偓婵?
    """
    s = JsonRpcServer()
...
class TestConversationMethods:
    ...
    async def test_list_conversations_returns_seeded(self, server: JsonRpcServer, agent: AIAgent) -> None:
        """闂佺_绻愰悧蹇涘极?repo 闂佺儵鏅涢悺銊ф暜閹绢喖绠甸柟鐑樺灥瀵?2 婵?conversation,闂佸憡鍔曠粔鐑芥憘?RPC list 婵°倗濮撮惌渚€鎯佹径鎰_?""
...
class TestProviderMethods:
    async def test_list_providers_returns_seeded_mock(self, server: JsonRpcServer) -> None:
        """seed_if_empty 闂佸湱绮_敮鎺楀矗閸℃_鈻旈柍褜鍓氱粙?mock entry闂?""
        ...
        # seed_if_empty 婵☆偓绲鹃悧鏇㈠箚鎼淬劍鍎庨悗娑櫭_径宥夋煙?mock 闁荤姳鑳堕崑鎾诲垂濮橆収娓舵俊顖涱儥閸?provider
    ...
    async def test_add_provider_duplicate_returns_err_duplicate(
        ...
        """seed 閻庤_鐡曠亸娆戝垝閿熺姴瀚?'mock',闂佸憡鍔曠粔瀛樻叏閻愬搫瑙﹂悘鐐跺Г閸?闂?ERR_DUPLICATE闂?""
        line = await _call(
            server,
            "add_provider",
...
class TestLogMethods:
    ...
    async def test_list_logs_since_filter(self, server: JsonRpcServer, agent: AIAgent) -> None:
        """since 闁哄_鏅涘ú锕傚箮?闂?闂佸憡鐟_禍锝囨崲?created_at > since 闂佹眹鍔岀€氼垶銆侀幋锕€违?""
        async with agent.session_maker() as session:
            repo = LogRepo(session)
            await repo.create(provider="mock", status="ok")

        # 闂佸憡鐟﹂悧鏃傜博鐎涙_鈻旀い蹇撴搐娴犳劙鎮楃憴鍕__闁靛洤娲︾粋宥嗘償閿濆懐鐤€闂佸搫鐗嗛ˇ鎶姐€?created_at 闂?since,闁圭厧鐡ㄥΛ浣烘崲閹寸姷鐭?
...
class TestRegistration:
    ...
    def test_register_methods_all_21_present(self, server: JsonRpcServer) -> None:
        """register_methods 闁圭厧鐡ㄥΛ渚€顢氬_顑芥灃闁靛_鍎遍弬鈧?21 婵?method 闂佸憡鑹剧粔鏌ュ焵?""
        expected = {
            "chat",
            "list_conversations",
            "get_conversation",
...
#
# 濠碘剝顨呴惌鍌氼焽閹殿喚鍗?Chat 婵＄偑鍊濆_褔顢氶_绛嬬€?CLI 闂?`--base-url` / `--api-key`:RPC params 婵犮垼鍩栫喊宥囨暜鐎涙_鈻旈柕鍫濆_閸?
# 闂佸憡鐟_崹鍫曞焵椤掆偓椤︻垶鎮鸿_閳?婵炲_濮鹃_濠勭博閹绢喗顥堥柣鎰_暯閺?闂?ChatMethod 闁?AgentRegistry.reserve(session_key 闂?
...
#
# 婵°倗濮撮惌渚€鎯佹径鎰__闁圭偓娼欓·?`AgentRegistry.size()` 闂佺偨鍎查弻锟犲焵?default agent 婵炴垶鎸哥粔鏉戯耿?registry 闂?闂佸湱顣介崑鎾趁?size
# 闁诲海鎳撻張顒勫汲閿濆_鐭楃€广儱妫欒〖"per-call agent 缂傚倸鍊归幐鎼佹偤閵娾晛瀚夊_璺侯煬濡__鎮樿箛鏂跨仸婵?闂?
# ---------------------------------------------------------------------------
...
class TestChatPerCallOverride:
    ...
    async def test_same_override_hits_lru_cache(self, server: JsonRpcServer) -> None:
        """闂?(provider, base_url, api_key) 缂備焦顨忛崗娑氳姳閳轰讲鏋庨梽鍥_儍閻斿吋鍋ㄩ柕濞垮劜閸ゆ帒鈽夐幙鍐ㄥ箻缂佹唻濡囬埀?size 婵炴垶鎸哥粔纾嬨亹婢舵劕违?""
    ...
    async def test_different_override_creates_separate_agents(self, server: JsonRpcServer) -> None:
        """婵炴垶鎸哥粔鎾_箖?base_url 闂?闂佸憡鑹剧€氼垶宕靛_鍛_＝闁规儳纾_幗?session_key hash 婵炴垶鎸哥粔鐢稿箰閹烘_违?""
