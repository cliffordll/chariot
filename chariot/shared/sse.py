...
`SseParser` 是 `chariot/shared/` 下的共享工具:server agent 转发上游 Model SSE
时复用,SDK ChatStream 解析 client-side 流时复用。0.5.0 起 server 也消费它
(协议级流式工具循环),所以从原位置 `chariot.sdk.streams` 提到 `chariot.shared`
(零依赖,既不属 server 也不属 sdk),避免 `agent → sdk.streams → sdk.__init__
→ sdk.client → server.controller → dataplane → agent` 的循环 import。

外部用 `SseParser.iter_frames(byte_iter)`(参数是 `AsyncIterable[bytes]`,
httpx response 传 `resp.aiter_bytes()`,server StreamingResponse 传
`resp.body_iterator`);单元测试可直接调 `SseParser._parse_frame`。
