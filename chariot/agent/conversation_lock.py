...
- **进程内**:`asyncio.Lock` per conv_id —— 同进程内同 conv 串行
- **跨进程**:SQLite `BEGIN IMMEDIATE` 短事务
  (`ConversationRepo.with_advisory_lock`) —— 多进程同 conv 串行写

旧 0.5.0 只有进程内锁(单 server 进程模式);0.6.0 库化后多 surface 进程
(CLI + Gateway + sidecar)可能同时写同 conv,加 SQLite 层保护。
...
- **DB-level lock(SQLite BEGIN IMMEDIATE)**:多进程并发触发;0.6.0 新增。
  实现见 `ConversationRepo.with_advisory_lock`(短事务 + busy_timeout 重试)
...
class ConversationLockManager:
    """每 conv_id 一把双层锁(进程内 + 可选 SQLite advisory)。
    ...
    错误形态:
    - 进程内锁超时 → `ServiceError(503, "conversation_busy")`(0.5.0 行为,
      过渡期保留;controller 层期望此异常)
    - DB 锁超时 → `ConversationLockTimeout(layer="db")`(0.6.0 新)
    """
        ...
        *,
        ...
        try:
            if db_session is None:
                ...
                async with ConversationRepo(db_session).with_advisory_lock(
                    conv_id, timeout_s=timeout_s
                ):
                    yield
        finally:
            lock.release()
